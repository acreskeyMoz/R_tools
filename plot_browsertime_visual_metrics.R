...
setwd("/users/acreskey/tools/R/data/gve_and_chrome")

dir.create("plots")
system('bash ../to_csv_visual_metrics.sh')
...
title <- "cold page loads. network conditions: rtt=100ms"
...
ggplot(df, aes(x=url, y=speedIndex)) +labs(title=title, y = "speedIndex") + geom_point(aes(color=mode), position = position_dodge(width=0.6)) + theme(axis.text.x=element_text(angle=-67.5, hjust=0, vjust=0))
#ggplot(df, aes(x=url, y=loadtime)) +labs(title="", y = "loadtime ms") + geom_dotplot(aes(fill=mode), dotsize = .1, position_dodge(width=2.0), binaxis = "y", stackdir = "center") + theme(axis.text.x=element_text(angle=-67.5, hjust=0, vjust=0))
